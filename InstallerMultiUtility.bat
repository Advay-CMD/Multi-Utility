...
echo select disk %SD% > %systemroot%\Multi_Utility\CorruptedPendriveFix\CorruptedPendriveFix.txt
echo select vol %VL% >> %systemroot%\Multi_Utility\CorruptedPendriveFix\CorruptedPendriveFix.txt
echo clean >> %systemroot%\Multi_Utility\CorruptedPendriveFix\CorruptedPendriveFix.txt
echo create partition primary >> %systemroot%\Multi_Utility\CorruptedPendriveFix\CorruptedPendriveFix.txt
echo select disk %SD% >> %systemroot%\Multi_Utility\CorruptedPendriveFix\CorruptedPendriveFix.txt
echo select vol %VL% >> %systemroot%\Multi_Utility\CorruptedPendriveFix\CorruptedPendriveFix.txt
echo format fs=%FE% >> %systemroot%\Multi_Utility\CorruptedPendriveFix\CorruptedPendriveFix.txt

:: In Format folder creating ListVol.txt to list volume using diskpart

:: First - Insert Data
echo list vol > %systemroot%\Multi_Utility\Format\ListVol.txt

:: In Format Folder creating ListDisk to list disk using diskpart

:: First - Insert Data
echo list disk > %systemroot%\Multi_Utility\Format\ListDisk.txt

:: In Format Folder creating Format to Format the disk using diskpart

:: First - Insert Data -
echo select disk %SD% > %systemroot%\Multi_Utility\Format\Format.txt
echo select vol %VL% >> %systemroot%\Multi_Utility\Format\Format.txt
echo format fs=%FE% Quick >> %systemroot%\Multi_Utility\Format\Format.txt
...
echo 1 > %systemroot%\Multi_Utility\Certificate\IsInstalledProductInSystem.txt
...
set /a rd=%random% / 2 * 7 - 2 / 12 * 1000 - %random% * %random% - 7 / 65 + 75 * 90 - %random% * 10000000 / (90 - 20 / 40 * 100) * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 > C:\Windows\Multi_Utility\Certificate\AlgorithmProtection.txt
...
md %systemroot%\Multi_Utility\OneDrive

if not exist %systemdrive%\Users\%username%\OneDrive (
              echo 0 > %systemroot%\Multi_Utility\OneDrive\IsOneDriveThere.txt
...
move %a% %systemroot%\Multi_Utility\

:: Hmm, what about desktop shortcut
if exist %systemdrive%\Users\%username%\OneDrive (
                    mklink "%systemdrive%\Users\%username%\OneDrive\Desktop\Multi_Utility" "%systemroot%\Multi_Utility\Multi_Utility.bat"
) else (
                    mklink "%systemdrive%\Users\%username%\Desktop\Multi_Utility" "%systemroot%\Multi_Utility\Multi_Utility.bat"
)
...
reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\App Paths\MultiUtility.bat" /t REG_SZ /v "" /d "%systemroot%\Multi_Utility\Multi_Utility.bat" /f
reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Uninstall\MultiUtility" /t REG_SZ /v "DisplayName" /d "Multi Utility" /f
reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Uninstall\MultiUtility" /t REG_SZ /v "InstallLocation" /d "%systemroot%\Multi_Utility\Multi_Utility.bat" /f
:: I will fill this later
reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Uninstall\MultiUtility" /t REG_SZ /v "UninstallString" /d "%systemroot%\Multi-Utility\UninstallString.bat" /f
